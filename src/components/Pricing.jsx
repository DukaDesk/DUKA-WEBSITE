...
import { NAVY, AMBER, WHITE, CREAM, SURFACE, BORDER, CHAR, LGREY, INK, ISLAND, ISLAND_INK, ISLAND_MUTE, GREEN, sora, mix } from "../constants";
...
<section id="pricing" data-pad="wide" style={{ padding: "100px 80px", background: CREAM }}>
      <FadeIn>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <h2 data-type="h2" style={{ fontFamily: sora, fontWeight: 800, fontSize: 48, color: INK, marginBottom: 12 }}>No surprises. No commissions.</h2>
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 12, background: SURFACE, borderRadius: 32, padding: "6px 20px", border: `1px solid ${BORDER}` }}>
            <span style={{ fontSize: 14, color: !annual ? INK : LGREY, fontWeight: !annual ? 600 : 400 }}>Monthly</span>
            ...
            <span style={{ fontSize: 14, color: annual ? INK : LGREY, fontWeight: annual ? 600 : 400 }}>Annual <span style={{ color: GREEN, fontWeight: 600, fontSize: 12 }}>Save 20%</span></span>
          </div>
        </div>
      </FadeIn>
        ...
        {plans.map((plan, i) => (
          <FadeIn key={i} delay={i * 100}>
            <Tilt max={plan.highlight ? 4 : 6} style={{ height: "100%" }}>
              <div style={{ background: plan.highlight ? ISLAND : SURFACE, borderRadius: 24, padding: "36px 32px", border: plan.highlight ? "none" : `1.5px solid ${BORDER}`, position: "relative", transform: plan.highlight ? "scale(1.04)" : "none", boxShadow: plan.highlight ? "0 24px 64px rgba(26,26,46,0.35)" : "none", height: "100%" }}>
              ...
              <div style={{ fontFamily: sora, fontWeight: 700, fontSize: 18, color: plan.highlight ? ISLAND_INK : INK, marginBottom: 6 }}>{plan.name}</div>
              <div style={{ fontSize: 13, color: plan.highlight ? ISLAND_MUTE : LGREY, marginBottom: 4 }}>{plan.desc}</div>
              <div style={{ fontSize: 12, color: plan.highlight ? mix("amber", 88) : LGREY, marginBottom: 20, fontStyle: "italic" }}>{plan.value}</div>
              <div style={{ fontFamily: sora, fontWeight: 800, fontSize: 36, color: plan.highlight ? AMBER : INK, marginBottom: 4 }}>{plan.price}</div>
              <div style={{ fontSize: 13, color: plan.highlight ? ISLAND_MUTE : LGREY, marginBottom: 28 }}>{plan.period}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 32 }}>
                {plan.features.map((f, fi) => (
                  <div key={fi} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                    ...
                    <span style={{ fontSize: 14, color: plan.highlight ? mix("island-ink", 75) : CHAR }}>{f}</span>
                  </div>
                ))}
              </div>
      ...
      <FadeIn delay={300}>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", background: SURFACE, borderRadius: 24, padding: "32px 36px", border: `1px solid ${BORDER}` }}>
          <div style={{ fontFamily: sora, fontWeight: 700, fontSize: 18, color: INK, marginBottom: 20, textAlign: "center" }}>Compare plans side by side</div>
          <div data-grid="compare" style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 8 }}>
            ...
            {compareFeatures.map((row, i) => {
              ...
              <div key={i} style={{ display: "contents" }}>
                <div style={{ fontSize: 13, color: INK, padding: "10px 8px", borderTop: `1px solid ${BORDER}`, fontWeight: 500 }}>{row.name}</div>
                <div style={{ fontSize: 13, color: LGREY, textAlign: "center", padding: "10px 8px", borderTop: `1px solid ${BORDER}` }}>{cell(row.starter)}</div>
                <div style={{ fontSize: 13, color: INK, textAlign: "center", padding: "10px 8px", borderTop: `1px solid ${BORDER}`, fontWeight: 600 }}>{cell(row.growth)}</div>
