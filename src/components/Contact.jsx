...
import { NAVY, AMBER, CREAM, BORDER, LGREY, SURFACE, INK, sora, inter, tint } from "../constants";
...
export function Contact() {
  return (
    <section id="contact" data-pad="wide" style={{ padding: "100px 80px 80px", background: SURFACE, minHeight: "100vh" }}>
      <FadeIn>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 data-type="h2" style={{ fontFamily: sora, fontWeight: 800, fontSize: 48, color: INK, marginBottom: 16 }}>
            We're here to help.
          </h2>
      ...
      <div data-grid="3" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, maxWidth: 960, margin: "0 auto" }}>
        {[
          ...
          const inner = (
            <>
              <div style={{ width: 52, height: 52, background: tint(AMBER, 18), borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
              ...
              <div style={{ fontFamily: sora, fontWeight: 700, fontSize: 17, color: INK, marginBottom: 6, wordBreak: "break-word" }}>{c.value}</div>
              ...
              {c.href ? (
                <a href={c.href} style={{ textDecoration: "none", display: "block" }}>
                  <div style={{ background: CREAM, borderRadius: 20, padding: "32px 28px", border: `1px solid ${BORDER}`, height: "100%", transition: "all 0.2s" }}
                    onMouseOver={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = "0 16px 40px rgba(0,0,0,0.08)"; e.currentTarget.style.borderColor = tint(AMBER, 44); }}
      ...
      <FadeIn delay={250}>
        <div style={{ textAlign: "center", marginTop: 56 }}>
          <div style={{ fontFamily: sora, fontWeight: 600, fontSize: 14, color: INK, marginBottom: 14 }}>Or find us on social</div>
          <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
            {SOCIALS.map((s, i) => (
              <a key={i} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.aria} style={{ width: 38, height: 38, background: NAVY, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#FFFFFF", cursor: "pointer", transition: "all 0.2s", textDecoration: "none" }}
                onMouseOver={e => { e.currentTarget.style.background = AMBER; e.currentTarget.style.color = NAVY; }}
                onMouseOut={e => { e.currentTarget.style.background = NAVY; e.currentTarget.style.color = "#FFFFFF"; }}
              >{s.label}</a>
            ))}
          </div>
