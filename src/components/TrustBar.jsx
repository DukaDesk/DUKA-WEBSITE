...
import { NAVY, CHAR, CREAM, BORDER, LGREY, AMBER, WHITE, GREEN, sora } from "../constants";
...
function Stat({ value, label }) {
  ...
  return (
    <div ref={ref} style={{ textAlign: "center" }}>
      <div style={{ fontFamily: sora, fontWeight: 800, fontSize: 28, color: NAVY }}>{prefix}{count}{suffix}</div>
    ...
    <>
      <div data-pad="narrow" style={{ background: NAVY, padding: "40px 80px" }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, textAlign: "center", maxWidth: 600, margin: "0 auto" }}>
          DukaDesk is the all-in-one platform built specifically for African businesses. Sell more, save time, and grow faster — without switching between multiple tools.
        </p>
      </div>
      ...
      <div data-pad="narrow" style={{ background: WHITE, padding: "32px 80px", borderBottom: `1px solid ${BORDER}`, overflow: "hidden" }}>
          ...
          {[...SECURITY_BADGES, ...SECURITY_BADGES].map((b, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, paddingRight: 72 }}>
                ...
                <div>
                  <div style={{ fontFamily: sora, fontWeight: 600, fontSize: 13, color: NAVY }}>{b.label}</div>
